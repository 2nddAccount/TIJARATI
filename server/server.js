...
// Also allow repo-root .env when server is started from /server
...
// Legacy public UI removed to avoid entrypoint confusion.
...
// Legacy UI route removed intentionally to avoid editing the wrong UI.
...
app.post('/api/ai', async (req, res) => {
    try {
        ...
        // Build data summary from stored transactions
        ...
        const topItems = (() => {
            ...
            return Array.from(counts.entries()).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([k, v]) => ({ item: k, qty: v }));
        })();
